...
[![Tests Status](https://img.shields.io/badge/Tests-136%2F136%20Passed-success.svg)](#-test-suite--penetration-testing)
...
MCP Shield includes an exhaustive automated test suite covering all 55 adversarial attack vectors, GitHub PAT / OpenAI / AWS token DLP blocks, JSON Unicode escape normalization (`\uXXXX`), RSA private key blocks, SSRF / IMDS protections, Shell network egress blocking, MySQL conditional comments, CTE DML blocks, and Ed25519 cryptographic attestations with nonce and timestamp context binding:

```bash
# Run all workspace test suites
npm run test:all

# Run 80-vector adversarial penetration and evasion audit
...
✓ Penetration Audit Suite    ──► 81 Tests (75 Attack Vectors + Unicode Sanitization + Crypto Verification + Payload Validation) (PASSED)
─────────────────────────────────────────────────────────────────────────────
TOTAL PASSED: 136 / 136 Tests (100% Passed across entire platform)
```
